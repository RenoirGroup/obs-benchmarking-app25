...
if authentication_status:
    ...
    def get_db_engine():
        try:
            ...
            st.error(f"Error connecting to the database. Error: {e}")
            return None
    ...
    ADMIN_USERNAMES = ["ksymondson", "admin"] # IMPORTANT: Change ksymondson to your admin username
    if username in ADMIN_USERNAMES:
        st.sidebar.markdown("---")
        st.sidebar.header("Admin Panel")
        
        if 'show_register_user' not in st.session_state:
            st.session_state.show_register_user = False

        if st.sidebar.button("Add New User"):
            st.session_state.show_register_user = not st.session_state.show_register_user
        
        if st.session_state.show_register_user:
            try:
                if authenticator.register_user('Register new user', preauthorization=False):
                    st.success('User registered successfully!')
                    # Update the config file after registration
                    with open('config.yaml', 'w') as file:
                        yaml.dump(config, file, default_flow_style=False)
                    st.session_state.show_register_user = False # Hide form after success
                    st.rerun()
            except Exception as e:
                st.error(e)
